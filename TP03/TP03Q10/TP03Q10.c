...
void mostrar() {
    ...
    for (i = primeiro; i != NULL; i = i->prox) {
        printf("[%d ## ", count);
        // Assume imprimir is a function to print a Personagem
        imprimir(i->elemento);
        count++;
    }
    printf("[ Bottom ]\n");
...
int main(){
    ...
    for (int i = 0; i < N; i++) {
        ...
        if(strcmp(values[0], "I") == 0) {
            inserir(buscarPersonagem(personagens, values[1])); //inserirInicio
        } else if(strcmp(values[0], "R") == 0) {
            Personagem removido = remover(); //removerInicio
            printf("(R) %s\n", removido.name);
        }
    }
