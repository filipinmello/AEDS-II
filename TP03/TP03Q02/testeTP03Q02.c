...
int main(){
    ...
    for (int i = 0; i < N; i++) {
        ...
        tempToken = strtokSingle(input, " ");           //String[] values = line.split(" ", 3);
        while (tempToken != NULL && k < MAX_TOKENS) {
            tokens[k] = tempToken;
            k++;
            tempToken = strtokSingle(NULL, " ");
        }
        ...
        if(strcmp(tokens[0], "II") == 0) {
            inserir(buscarPersonagem(personagens, tokens[1]), 0); //inserirInicio
        } else if(strcmp(tokens[0], "IF") == 0) {
            inserir(buscarPersonagem(personagens, tokens[1]), n-1); //inserirFim
        } else if(strcmp(tokens[0], "I*") == 0) {
            int index = atoi(tokens[1]);
            inserir(buscarPersonagem(personagens, tokens[2]), index);
