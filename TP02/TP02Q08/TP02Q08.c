...
int compareEyeColour(Personagem p1, Personagem p2){
...
void insercaoPorCor(Personagem *inputPersonagens, int inputCount, int cor, int h){
    for (int i = (h + cor); i < inputCount; i+=h) {
        Personagem tmp = inputPersonagens[i];
        int j = i - h;
        while ((j >= 0) && (compareEyeColour(inputPersonagens[j], tmp) > 0)) {
            inputPersonagens[j + h] = inputPersonagens[j];
            j-=h;
        }
        inputPersonagens[j + h] = tmp;
    }
}
...
void sort(Personagem *inputPersonagens, int inputCount) {
    ...
    do { h = (h * 3) + 1; } while (h < inputCount);

    do {
        h /= 3;
        for(int cor = 0; cor < h; cor++){
            insercaoPorCor(inputPersonagens, inputCount, cor, h);
        }
    } while (h != 1);
}
