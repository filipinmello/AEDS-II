...
void selectionSort(Personagem *inputPersonagens, int inputCount, int index) {
    if (index >= inputCount - 1) 
        return;

    int menor = index;
    for (int j = index + 1; j < inputCount; j++) {
        if (strcmp(inputPersonagens[j].name, inputPersonagens[menor].name) < 0) {
            menor = j;
        }
    }
    swap(inputPersonagens, menor, index);

    selectionSort(inputPersonagens, inputCount, index + 1);
}

void sort(Personagem *inputPersonagens, int inputCount) {
    selectionSort(inputPersonagens, inputCount, 0); 
}
...
int main(){
    ...
    FILE *fileLog = fopen("827761_selecaoRecursiva.txt", "w");
    if (fileLog == NULL) {
